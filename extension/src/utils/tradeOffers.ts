    ...
    return new Promise((resolve, reject) => {
        chrome.storage.local.get(['steamSessionID'], ({ steamSessionID }) => {
            ...
            const request = new Request('https://steamcommunity.com/tradeoffer/new/send', {
                method: 'POST',
                headers: myHeaders,
                body: `sessionid=${steamSessionID}&serverid=1&partner=${getProperStyleSteamIDFromOfferStyle(
                    partnerID,
                )}&tradeoffermessage=${message}&json_tradeoffer=${JSON.stringify(
                    tradeOfferJSON,
                )}&captcha=&trade_offer_create_params=${JSON.stringify({
...
const notifyAboutOffer = (offer) => {
    chrome.storage.local.get('currency', ({ currency }) => {
        ...
        getPlayerSummaries([steamIDOfPartner]).then((summary) => {
            ...
            getRemoteImageAsObjectURL(userDetails.avatar)
                .then((iconURL) => {
                    icon = iconURL;
                })
                .catch((e) => {
                ...
                .finally(() => {
                    chrome.notifications.create(
                        `offer_received_${offer.tradeofferid}`,
                        {
                            type: 'basic',
                            iconUrl: icon,
                            title: `Offer from ${userDetails.personaname} (${prettyPrintPrice(
                                currency,
...
const notifyAboutOfferOnDiscord = (offer, items) => {
    chrome.storage.local.get('currency', ({ currency }) => {
        ...
        getPlayerSummaries([steamIDOfPartner]).then((summary) => {
            ...
            if (giving)
                fields.push({
                    name: `Giving (${prettyPrintPrice(currency, offer.yourItemsTotal.toFixed(2))})`,
                    inline: false,
                    value: giving,
                });
            if (receiving)
...
const executeVerdict = (offer, ruleNumber, verdict, items) => {
    ...
    switch (verdict) {
        case actions.notify.key:
            notifyAboutOffer(offer);
            break;
        case actions.notify_discord.key:
            notifyAboutOfferOnDiscord(offer, items);
            break;
        case actions.decline.key:
...
const evaluateCondition = (offer, condition) => {
    ...
    if (
        !(
            (offer.items_to_receive === undefined || offer.items_to_receive.length === 0) &&
        ...
        if (
            condition.type === conditions.profit_percentage_over.key &&
            offer.PLPercentage >= condition.value / 100 + 1
        ) {
            return true;
        }
        if (
            condition.type === conditions.profit_percentage_under.key &&
            offer.PLPercentage < condition.value / 100 + 1
        ) {
            return true;
        }
        ...
        if (
            condition.type === conditions.message_doesnt_include.key &&
            offer.message !== '' &&
            !offer.message.includes(condition.value)
        ) {
            return true;
        ...
        if (
            condition.type === conditions.giving_items_over.key &&
            ((offer.items_to_give !== undefined && offer.items_to_give.length >= condition.value) ||
                (condition.value <= 0 && offer.items_to_give === undefined))
        ) {
            return true;
        }
        if (
            condition.type === conditions.giving_items_under.key &&
            ((offer.items_to_give !== undefined && offer.items_to_give.length < condition.value) ||
...
const evaluateRule = (offer, rule) => {
    for (let i = 0; i < rule.conditions.length; i += 1) {
        ...
        } else if (rule.operators[i] === undefined || rule.operators[i] === operators.and.key) {
            return false;
        }
    }
    return false;
                    ...
                    .then((offersData) => {
                        ...
                        if (offersData.trade_offers_received) {
                            offersData.trade_offers_received.forEach((offer) => {
                                if (
                                    !prevProcessedOffersIDs.includes(offer.tradeofferid) &&
