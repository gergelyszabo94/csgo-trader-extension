...
if (document.querySelector('body').classList.contains('profile_page')) {
    ...
    if (commentThreadEntryBox !== null)
        commentThreadEntryBox.setAttribute('style', 'width: 83%; display: inline-block;');
    ...
    if (loggedInUserID === profileOwnerSteamID) {
        ...
        // when on someone else's profile that is not private
      ...
      <a class="popup_menu_item" href="https://csgo-rep.com/profile/${profileOwnerSteamID}">
        ...
        document.getElementById('copy_trade_link').addEventListener('click', () => {
            ...
            let tradeLink = `https://steamcommunity.com/tradeoffer/new/?partner=${getOfferStyleSteamID(
                profileOwnerSteamID,
            )}`;
    ...
    chrome.storage.local.get(
        ...
        ({ nsfwFilter, removeAnimatedProfileBackgrounds }) => {
            if (nsfwFilter) {
                ...
                document
                    .querySelectorAll(
                        '.profile_background_holder_content, .screenshot_showcase, .profile_animated_background',
                    )
                    .forEach((element) => {
                        element.remove();
                    });
                ...
                document
                    .querySelector('.playerAvatarAutoSizeInner')
                    .querySelector('img')
                    .setAttribute(
                        'src',
