                ...
                fetch(getRequest).then((response) => {
                    ...
                    if (body.body === undefined || body.body.success === 'false') {
                        if (retries < 5) setTimeout(() => {sendTelemetry(++retries)}, 600*5);
                            ...
                            const newAnalyticsEvents = result.analyticsEvents.filter(event => event.timestamp > (Date.now() - (1000 * 60 * 60 * 24 * 7)));
                            chrome.storage.local.set({analyticsEvents: newAnalyticsEvents}, () => {});
                        }
                    }
                    else if (body.body.success === 'true') chrome.storage.local.set({analyticsEvents: []}, () => {});
