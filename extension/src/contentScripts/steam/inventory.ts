...
const changeName = (name, color, appID, marketHashName, dopplerInfo) => {
    ...
    const newNameElement =
        dopplerInfo !== null && dopplerInfo !== undefined
            ...
            for (const ID of IDs) {
                if (!soldIDs.includes(ID)) {
                    ...
                    listItem(
                        ...
                        .then(() => {
                            const soldFromRow = document
                                .getElementById('listingTable')
                                .querySelector('.rowGroup')
                                .querySelector(`[data-item-name="${name}"]`);
...
const addRightSideElements = () => {
    ...
    if (activeIDs !== null) {
        ...
        if (
            activeInventoryAppID === steamApps.CSGO.appID ||
        ...
        if (item) {
            if (
                ...
                if (activeInventoryAppID === steamApps.CSGO.appID) {
                    ...
                    if (item.stickers !== undefined && item.stickers.length !== 0) {
                        ...
                        setTimeout(() => {
                            if (originalStickers !== null && originalStickers.parentNode !== null)
                                originalStickers.outerHTML = '';
                        }, 1000);
                    ...
                    chrome.storage.local.get(
                        [
                            'activeOffers',
                            'itemInOffersInventory',
                            'showPriceEmpireLinkInInventory',
                            'showBuffLookupInInventory',
                            'inventoryShowCopyButtons',
                        ],
                        ({
                            activeOffers,
                            itemInOffersInventory,
                                    ...
                                    document
                                        ...
                                        .forEach((descriptor) => {
                                            descriptor.insertAdjacentHTML(
                                                'afterend',
                                                DOMPurify.sanitize(inTradesInfoModule, {
                                                    ADD_ATTR: ['target'],
                                                }),
                                            );
                                        });
              ...
              `;
                                ...
                                document
                                    ...
                                    .forEach((descriptor) => {
                                        descriptor.insertAdjacentHTML(
                                            'afterend',
                                            DOMPurify.sanitize(priceEmpireLink, {
                                                ADD_ATTR: ['target'],
                                            }),
                                        );
                                    });
              ...
              `;
                                ...
                                document
                                    ...
                                    .forEach((descriptor) => {
                                        descriptor.insertAdjacentHTML(
                                            'afterend',
                                            DOMPurify.sanitize(buffLink, { ADD_ATTR: ['target'] }),
                                        );
                                    });
                            }

                            if (inventoryShowCopyButtons) {
                                document.querySelectorAll('.copyButtons').forEach((copyButtons) => {
                            ...
                            <div class="copyItemLink clickable" title="Cop the item's inventory link.">
                                ...
                                document.querySelectorAll('.copyItemLink').forEach((element) => {
                                    element.addEventListener('click', () => {
                                        copyToClipboard(
                                            `https://steamcommunity.com/profiles/${item.owner}/inventory/#${item.appid}_${item.contextid}_${item.assetid}`,
                                        );
                                    });
                                });
                            }
                    ...
                    if (item.floatInfo === null || item.floatInfo === undefined) {
                        ...
                        updateFloatAndPatternElements(item);
                        addFloatIndicator(
                            findElementByIDs(steamApps.CSGO.appID, '2', item.assetid, 'inventory'),
                            item.floatInfo,
                        );
                    }
                    ...
                    <div class="descriptor otherExteriors">
                        ...
                        genericMarketLink + linkMidPart + weaponName
                    }%28Battle-Scarred%29" target="_blank"><span class="${stOrSvClass} exteriorsLink">${stOrSv} ${
                        exteriors.battle_scarred.localized_name
                    }</span></a></li>
                        </ul>
                ...
                document.querySelectorAll('.tradabilityDiv').forEach((tradabilityDiv) => {
                    if (item.tradability === 'Tradable') {
                        ...
                        tradabilityDiv.innerHTML = DOMPurify.sanitize(
            ...
            const name = getItemByIDs(
                getItemInfoFromPage(item.appid, item.contextid),
                item.appid,
                item.contextid,
                item.assetid,
            ).description.name;
                    ...
                    if (
                        ...
                        if (item.tradability === 'Tradable') {
                            itemElement.insertAdjacentHTML(
                                'beforeend',
                                DOMPurify.sanitize('<div class="perItemDate tradable">T</div>'),
                            );
                        } else if (item.tradability !== 'Not Tradable') {
                            itemElement.insertAdjacentHTML(
                                'beforeend',
...
const getListingRow = (appID, contextID, name) => {
    return document
        .getElementById('listingTable')
        .querySelector(`.row[data-item-name="${appID}_${contextID}_${name}"]`);
};
...
const addListingRow = (item) => {
    const row = `
          <div class="row" data-ids="${item.appid}_${item.contextid}_${
        item.assetid
    }" data-sold-ids="" data-item-name="${item.appid}_${item.contextid}_${item.market_hash_name}">
                ...
                .forEach((itemElement) => {
                    ...
                    if (
                        ...
                        if (selected < quantity) {
                            if (!itemElement.classList.contains('cstSelected'))
                                itemElement.classList.add('cstSelected');
                            selected += 1;
                        } else if (itemElement.classList.contains('cstSelected'))
                            itemElement.classList.remove('cstSelected');
                    }
                });
...
const addStartingAtAndQuickSellPrice = (marketHashName, priceInCents, appID, assetID, contextID) => {
    ...
    if (listingRow !== null) {
        ...
        const extensionPrice = parseInt(
            listingRow.querySelector('.itemExtensionPrice').getAttribute('data-price-in-cents'),
        );
    ...
    if (listingRow !== null) {
        ...
        if (highestOrder !== undefined) {
            if (highestOrder === null) {
                // when there aren'y any buy orders for the item
...
const addMidPrice = (marketHashName, midPrice, appID, assetID, contextID) => {
    ...
    if (listingRow !== null) {
        ...
        if (midPrice !== undefined) {
            if (midPrice === null) {
                // when there aren't nay buy orders or listings
...
const listenSelectClicks = (event) => {
    if (
        event.target.parentElement.classList.contains('item') &&
...
const generateItemsList = () => {
    ...
    const sortedItems = doTheSorting(
        items,
...
const addFunctionBar = () => {
    if (document.getElementById('inventory_function_bar') === null) {
        ...
        document.getElementById('selectAllUnder').addEventListener('click', () => {
            ...
            document
                ...
                .forEach((itemElement) => {
                    ...
                    if (
                        item !== undefined &&
                        parseFloat(item.price.price) < underThisPrice &&
                        !itemElement.classList.contains('cstSelected') &&
                        item.marketable === 1
                    ) {
                        itemElement.classList.add('cstSelected');
                    }
        ...
        document.getElementById('generate_list').addEventListener('click', () => {
            document.getElementById('functionBarGenerateMenu').classList.toggle('hidden');
        });
            ...
            chrome.runtime.sendMessage({ hasTabsAccess: 'hasTabsAccess' }, (response) => {
                if (response) {
                    ...
                    if (dropDownMenu !== null && viewTradeHistory === null) {
                        dropDownMenu
                            .querySelector('.popup_body.popup_menu')
                            .insertAdjacentHTML(
                                'afterbegin',
                                DOMPurify.sanitize(
