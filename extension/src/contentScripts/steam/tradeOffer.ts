...
const getOfferIDScript =
    "document.querySelector('body').setAttribute('offerID', g_strTradePartnerInventoryLoadURL.split('tradeoffer/')[1].split('/partner')[0])";
...
const buildInventoryStructure = (inventory) => {
    ...
    return inventoryArrayToReturn.sort((a, b) => {
...
const addPLInfo = () => {
    ...
    if (yourTotalEl !== null && theirTotalEl !== null) {
        ...
        Show trade Summary (<span class="${pLClass}">${prettyPrintPrice(
                currency,
                profitOrLoss.toFixed(2),
            )}  ${getFormattedPLPercentage(yourTotal, theirTotal)}</span>)`);
        });
    }
};
    ...
    if (inventoryIDS.appID === steamApps.CSGO.appID) {
        if (type === 'offer') {
            ...
            doTheSorting(
                ...
                Array.from(items),
                method,
                document.getElementById(`${type}_slots`),
                type,
            );
            const inventoryTab = document.querySelector(`[href="#${type}_inventory"]`);
            if (inventoryTab !== null && inventoryTab.classList.contains('active'))
...
const singleClickControlClickHandler = (event) => {
    if (event.ctrlKey) {
        ...
        let inInventory;
        if (event.target.parentNode.parentNode.parentNode.parentNode.id === 'their_slots')
            inInventory = document.getElementById('their_slots');
        else if (event.target.parentNode.parentNode.parentNode.parentNode.id === 'your_slots')
            inInventory = document.getElementById('your_slots');
...
const addPartnerOfferSummary = () => {
    chrome.storage.local.get(
        ['tradeHistoryOffers', `offerHistory_${getTradePartnerSteamID()}`, 'apiKeyValid'],
        (result) => {
...
const sendQueryParamOffer = (urlParams: URLSearchParams, whose: string, items: SmallItem[], message: string) => {
    ...
    sendOffer(urlParams.get('partner'), tradeOfferJSON, urlParams.get('token'), message)
        .then(() => {
            closeTab();
        })
        .catch((err) => {
...
document
    .querySelectorAll('#inventory_select_their_inventory, #inventory_select_your_inventory')
                    ...
                    const selectInterval = setInterval(() => {
                        ...
                        if (type === 'id') {
                            ...
                            if (items.length === ids.length) {
                                // only move items if all are found
