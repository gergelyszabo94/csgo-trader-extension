...
const addElements = () => {
    ...
    else document.querySelectorAll('.countdown').forEach((countdown) => {
        countdown.style.display = 'none';
    });
};
...
const addBookmark = (module) => {
    ...
    chrome.storage.local.get('bookmarks', (result) => {
        chrome.storage.local.set({'bookmarks': [...result.bookmarks, bookmark]}, () => {
            if (bookmark.itemInfo.tradability !== 'Tradable') chrome.runtime.sendMessage({
                setAlarm: {
...
const hideOtherExtensionPrices = () => {
    // sih
    if (!document.hidden && isSIHActive()) document.querySelectorAll(".price_flag").forEach((price) => {
        price.remove();
    });
