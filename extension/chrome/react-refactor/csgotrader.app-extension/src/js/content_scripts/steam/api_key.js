...
chrome.storage.local.get('autoSetSteamAPIKey', (result) => {
    ...
    if (result.autoSetSteamAPIKey) {
        if (document.getElementById('editForm').action.includes('registerkey')) { // if no API key registered yet, registers one
            ...
            let apiKey = document.getElementById('bodyContents_ex').querySelector('p').innerText.split(': ')[1];
            chrome.runtime.sendMessage({apikeytovalidate: apiKey}, (response) => {
                if (response.valid){
                    chrome.storage.local.set({steamAPIKey: apiKey, apiKeyValid: true}, () => {
                        document.getElementById('editForm').insertAdjacentHTML('afterend', `<div style="color:green; margin-top: 10px;">Added API key to CSGOTrader Extension</div>`);
                        console.log('api key valid');
                    });
                }
                else{
                    document.getElementById('editForm').insertAdjacentHTML('afterend', `<div style="color:red; margin-top: 10px;">CSGOTrader Extension could not validate your API key, please try again.</div>`);
