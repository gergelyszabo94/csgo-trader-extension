...
function overrideShowTradeOffer(){
    let overRideShowTradeOfferScript = `
...
`;
    chrome.storage.local.get(['openOfferInTab'], (result) =>{if(result.openOfferInTab) injectToPage(overRideShowTradeOfferScript, false, 'ShowTradeOfferScript', null)});
}

// overrides steam's trade offer declining logic to skip the confirmation
function overrideDecline(){
    let overRideDeclineTradeOfferScript = `
    function DeclineTradeOffer( tradeOfferID )
    {
        ActOnTradeOffer( tradeOfferID, 'decline', 'Trade Declined', 'Decline Trade' );
    }
    `;
    chrome.storage.local.get(['quickDeclineOffer'], (result) =>{if(result.quickDeclineOffer) injectToPage(overRideDeclineTradeOfferScript, false, 'overrideDeclineTradeOffer', null)});
}

// adds In-browser inspect as action - in trade offers
function overrideHandleTradeActionMenu(){
    let overrideHandleTradeActionMenuScript = `
...
function overridePopulateActions(){
    let overRidePopulateActionsMenuScript = `
