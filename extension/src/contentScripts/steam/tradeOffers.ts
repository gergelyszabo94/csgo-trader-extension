...
if (window.location.href.includes('/tradeoffers/?history=1'))
    activePage = 'incoming_offers_history';
else if (window.location.href.includes('/tradeoffers/sent/?history=1'))
    activePage = 'sent_offers_history';
...
const addInOtherOffersInfoBlock = (item, otherOfferItems, offerElement) => {
    ...
    const otherOffers = otherOfferItems.map((otherOfferItem, index) => {
        ...
        if (activePage === 'incoming_offers') {
            offerLink =
                otherOfferItem.offerOrigin === 'sent'
        ...
        const afterLinkChars =
            index === otherOfferItems.length - 1
                ? '' // if it's the last one
                : index !== 0 && index % 4 === 0
                ? ', \n' // if it's the 4th, 8th, etc. add a new line since only 4 fits on a line
                : ', '; // normal case

        return `<a href="${offerLink}" ${
            otherOfferItem.offerOrigin === 'sent' ? 'target="_blank"' : ''
        }>
...
const addItemInfo = (items) => {
    ...
    document.querySelectorAll('.tradeoffer').forEach((offerElement) => {
        if (isOfferActive(offerElement)) {
            ...
            const yourSideItems = [...yourSide.querySelectorAll('.trade_item')].map(
                (itemElement, index) => {
                    return { itemElement, side: 'your', position: index };
                },
            );

            const theirSideItems = [...theirSide.querySelectorAll('.trade_item')].map(
                (itemElement, index) => {
    ...
    chrome.storage.local.get(
        ...
        ({
            ...
            activeItemElements.forEach(({ itemElement, side, position }) => {
                if (
                    itemElement.getAttribute('data-processed') === null ||
                    itemElement.getAttribute('data-processed') === 'false'
                ) {
                    const item = findItem(
                        items,
                        getLimitedIDsFromElement(itemElement),
                        side,
                        position,
                    );
                    if (item) {
...
const addTotals = (offers) => {
    chrome.storage.local.get('currency', ({ currency }) => {
        ...
        offers.forEach((offer) => {
            ...
            if (isOfferActive(offerElement)) {
                activeOfferCount += 1;
                const primaryHeader = offerElement
                    .querySelector('.tradeoffer_items.primary')
                    .querySelector('.tradeoffer_items_header');
                const secondaryHeader = offerElement
                    .querySelector('.tradeoffer_items.secondary')
                    .querySelector('.tradeoffer_items_header');
                const theirHeader =
                    activePage === 'incoming_offers' ? primaryHeader : secondaryHeader;
                const yourHeader =
                    activePage === 'incoming_offers' ? secondaryHeader : primaryHeader;

                theirHeader.innerText += ` ${prettyPrintPrice(
                    currency,
                    offer.theirItemsTotal.toFixed(2),
                )}`;
                theirHeader.innerText = offer.theirIncludesItemWIthNoPrice
                    ? (theirHeader.innerText += ' - includes items with no price')
                    : theirHeader.innerText;
                yourHeader.innerText += ` ${prettyPrintPrice(
                    currency,
                    offer.yourItemsTotal.toFixed(2),
                )}`;
                yourHeader.innerText = offer.yourIncludesItemWIthNoPrice
                    ? (yourHeader.innerText += ' - includes items with no price')
                    : yourHeader.innerText;
        ...
        document.getElementById('tradeoffers_summary').innerHTML = DOMPurify.sanitize(`
            ...
            <div id="potential_profit"><b>Potential profit: </b>${prettyPrintPrice(
                currency,
...
const sortOffers = (sortingMode) => {
    const activeOffers = [...document.querySelectorAll('.tradeoffer')].filter((offerElement) =>
        isOfferActive(offerElement),
    );
    let sortedOffers = [];

    if (sortingMode === offersSortingModes.profit_amount.key) {
        sortedOffers = activeOffers.sort((a, b) => {
            const profitOnA = parseFloat(
                a.querySelector('.profitOrLoss').getAttribute('data-profit-or-loss'),
            );
            const profitOnB = parseFloat(
                b.querySelector('.profitOrLoss').getAttribute('data-profit-or-loss'),
            );
        ...
        sortedOffers = activeOffers.sort((a, b) => {
            const pLA = parseFloat(
                a.querySelector('.profitOrLoss').getAttribute('data-p-l-percentage'),
            );
            const pLB = parseFloat(
                b.querySelector('.profitOrLoss').getAttribute('data-p-l-percentage'),
            );
            return pLB - pLA;
        });
        ...
        sortedOffers = activeOffers.sort((a, b) => {
            const updatedA = parseInt(
                a.querySelector('.profitOrLoss').getAttribute('data-updated'),
            );
            const updatedB = parseInt(
                b.querySelector('.profitOrLoss').getAttribute('data-updated'),
            );
        ...
        sortedOffers = activeOffers.sort((a, b) => {
            const updatedA = parseFloat(
                a.querySelector('.profitOrLoss').getAttribute('data-receiving-total'),
            );
            const updatedB = parseFloat(
                b.querySelector('.profitOrLoss').getAttribute('data-receiving-total'),
            );
            return updatedB - updatedA;
        });
        ...
        sortedOffers = activeOffers.sort((a, b) => {
            const updatedA = parseFloat(
                a.querySelector('.profitOrLoss').getAttribute('data-giving-total'),
            );
            const updatedB = parseFloat(
                b.querySelector('.profitOrLoss').getAttribute('data-giving-total'),
            );
            return updatedB - updatedA;
        });
    }
    ...
    document
        .querySelector('.profile_leftcol')
        .querySelectorAll('.tradeoffer_rule')
        .forEach((rulerElement) => {
            rulerElement.remove();
        });
...
const addPartnerOfferSummary = (offers) => {
    chrome.storage.local.get(
        ['tradeHistoryOffers', 'friendRequests'],
        ({ tradeHistoryOffers, friendRequests }) => {
            if (tradeHistoryOffers) {
                offers.forEach((offer) => {
                    ...
                    const offerElement = document.getElementById(
                        `tradeofferid_${offer.tradeofferid}`,
                    );
                    ...
                    if (friendRequestFromPartner.length === 1) {
                        ...
                        offerElement
                            .querySelector('.tradeoffer_header')
                            .insertAdjacentHTML(
                                'afterend',
                    ...
                    chrome.storage.local.get(storageKey, (result) => {
                        ...
                        if (offerHistorySummary !== undefined) {
                            ...
                            offerElement
                                .querySelectorAll('.offerHistory')
                                .forEach((offerHistoryElement) => {
                                    offerHistoryElement.remove();
                                });

                            if (isOfferActive(offerElement)) {
                                const receivedElement =
                                    offerHistorySummary.offers_received === 0
                                        ? '<span  class="offerHistory">Received: 0</span>'
                                        : `<span class="offerHistory">Received: ${
                                              offerHistorySummary.offers_received
                                          } Last: ${prettyTimeAgo(
                                              offerHistorySummary.last_received,
                                          )}</span>`;
                                offerElement
                                    .querySelector('.tradeoffer_items.primary')
                                    .insertAdjacentHTML(
                                        'beforeend',
                                        DOMPurify.sanitize(receivedElement),
                                    );

                                let sentElement = `<span  class="offerHistory">Sent: ${
                                    offerHistorySummary.offers_sent
                                } Last: ${prettyTimeAgo(offerHistorySummary.last_sent)}</span>`;
                                if (offerHistorySummary.offers_sent === 0)
                                    sentElement = '<span  class="offerHistory">Sent: 0</span>';
                                offerElement
                                    .querySelector('.tradeoffer_items.secondary')
                                    .insertAdjacentHTML(
                                        'beforeend',
                                        DOMPurify.sanitize(sentElement),
                                    );
                            }
                        }
                    });
            ...
            chrome.storage.local.get('tradeHistoryLastUpdate', (firstResult) => {
                ...
                allOffers.forEach((offer) => {
                    if (offer.time_updated > firstResult.tradeHistoryLastUpdate) {
                        const partnerID = getProperStyleSteamIDFromOfferStyle(
                ...
                for (const [steamID, offersToAdd] of Object.entries(offerHistoryToAdd)) {
                    ...
                    offersToAdd.forEach((offer) => {
                        if (offer.ours) {
                            ...
                            received += 1;
                            lastReceived =
                                offer.timestamp > lastReceived ? offer.timestamp : lastReceived;
                        }
                    });
                            ...
                            chrome.storage.local.set(
                                {
                                    [storageKey]: {
                                        offers_received:
                                            offerSummaryFromStorage.offers_received + received,
                                        offers_sent: offerSummaryFromStorage.offers_sent + sent,
                                        last_received:
                                            lastReceived > offerSummaryFromStorage.last_received
                                                ? lastReceived
                                                : offerSummaryFromStorage.last_received,
                                        last_sent:
                                            lastSent > offerSummaryFromStorage.last_sent
                                                ? lastSent
...
const periodicallyUpdateRealTimeTotals = (offerEl) => {
    setInterval(() => {
        chrome.storage.local.get(['userSteamWalletCurrency'], ({ userSteamWalletCurrency }) => {
            ...
            if (primaryTotalEl !== null) primaryTotalEl.innerText = primaryTotalFormatted;
            else {
                offerPrimarySide.insertAdjacentHTML(
                    'beforeend',
                    DOMPurify.sanitize(
            ...
            if (secondaryTotalEl !== null) secondaryTotalEl.innerText = secondaryTotalFormatted;
                ...
                offerSecondarySide.insertAdjacentHTML(
                    'beforeend',
                    DOMPurify.sanitize(
...
else if (activePage === 'incoming_offers_history')
    changePageTitle('trade_offers', 'Incoming Trade Offers History');
else if (activePage === 'sent_offers_history')
    changePageTitle('trade_offers', 'Sent Trade Offers History');
            ...
            document.querySelectorAll('.tradeoffer_items.secondary').forEach((secondaryElement) => {
                secondaryElement.querySelectorAll('.trade_item').forEach((itemElement) => {
                    ...
                    iconElement.setAttribute(
                        'srcset',
                        iconElement.getAttribute('srcset').replace('73fx73f', '96fx96f'),
                    );
                });
            });
        }
...
if (activePage === 'incoming_offers') {
    ...
    document.querySelectorAll<HTMLElement>('.tradeoffer_items_rule').forEach((rule) => {
    ...
    document.querySelectorAll('.tradeoffer').forEach((offerElement) => {
        if (isOfferActive(offerElement)) {
            const offerID = getOfferIDFromElement(offerElement);
            const partnerID = getProperStyleSteamIDFromOfferStyle(
                offerElement.querySelector('.playerAvatar').getAttribute('data-miniprofile'),
            );
            offerElement
                .querySelector('.tradeoffer_footer_actions')
                .insertAdjacentHTML(
                    'afterbegin',
    ...
    chrome.storage.local.get(['quickDeclineOffer'], ({ quickDeclineOffer }) => {
        if (quickDeclineOffer) {
            document.querySelectorAll('.tradeoffer_footer_actions').forEach((actionsEl) => {
                const actions = actionsEl.querySelectorAll<HTMLElement>('a.whiteLink');
...
if (activePage === 'incoming_offers' || activePage === 'sent_offers') {
    ...
    chrome.storage.local.get('itemInOtherOffers', ({ itemInOtherOffers }) => {
        if (itemInOtherOffers) {
            ...
            const buttons = [];
            document
                .querySelectorAll('.tradeoffer_footer_actions')
                .forEach((offerActionsElement) => {
    ...
    if (tradeOffersList !== null && document.querySelector('.profile_fatalerror') === null) {
        ...
        const sortingSelect = document.getElementById('offerSortingMethod') as HTMLSelectElement;
