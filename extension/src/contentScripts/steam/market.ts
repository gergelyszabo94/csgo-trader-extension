...
const createCSV = () => {
    ...
    let csvContent =
        'Item Name,Game Name,Listed On,Acted On, Display Price, Price in Cents, Type, Partner Name, Partner Link\n';
            ...
            (history) => {
                ...
                if (marketHistoryExport.progress >= marketHistoryExport.to - marketHistoryExport.from) {
                    ...
                    document.getElementById('exportHelperMessage').innerText =
                        'Export finished, you can now download the result!';
    ...
    if (tableHeader !== null) {
        ...
        if (tabContentRows !== null) {
            ...
            const observer = new MutationObserver(() => {
                if (sellListings.parentElement.style.display !== 'none') {
                    // only execute if it's the active tab
        ...
        removeColumnHeader.insertAdjacentHTML(
            'afterend',
            DOMPurify.sanitize(`
...
if (orders) {
    ...
    if (orderRows.length !== 0) {
        chrome.storage.local.get(['outBidPercentage'], ({ outBidPercentage }) => {
            ...
            const remainingAmountForOrders =
                totalOrderAmount > totalAllowedByWallet
                    ? ''
                    : `You can set more orders totaling: ${centsToSteamFormattedPrice(
                          totalAllowedByWallet - totalOrderAmount,
                      )}`;
            orders.insertAdjacentHTML(
                ...
                DOMPurify.sanitize(
                   ...
                   <span
                    ...
                    title="${
                        totalOrderAmount > totalAllowedByWallet
                            ? "You can't place new orders right now!"
                            : 'You can place new orders up to this amount!'
                    }">
...
if (marketHistoryTab !== null) {
    ...
    const observer = new MutationObserver((mutationRecord) => {
        if (sellListings.style.display !== 'none') {
            // only execute if it's the active tab
            if (
                mutationRecord[0].target.id === 'tabContentsMyMarketHistory' ||
