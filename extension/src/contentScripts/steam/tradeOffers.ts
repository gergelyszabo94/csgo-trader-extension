                ...
                chrome.storage.local.get(storageKey, (result) => {
                    ...
                    if (offerHistorySummary !== undefined) {
                        ...
                        if (isOfferActive(offerElement)) {
                            const receivedElement =
                                offerHistorySummary.offers_received === 0
                                    ...
                                    : `<span class="offerHistory">Received: ${
                                      ...
                                      } Last: ${prettyTimeAgo(offerHistorySummary.last_received)}</span>`;
                            offerElement
                                .querySelector('.tradeoffer_items.primary')
                                .insertAdjacentHTML('beforeend', DOMPurify.sanitize(receivedElement));
                            ...
                            offerElement
                                .querySelector('.tradeoffer_items.secondary')
                                .insertAdjacentHTML('beforeend', DOMPurify.sanitize(sentElement));
                        }
                    }
                });
            });
...
const periodicallyUpdateRealTimeTotals = (offerEl) => {
    setInterval(() => {
        chrome.storage.local.get(['userSteamWalletCurrency'], ({ userSteamWalletCurrency }) => {
            ...
            const primaryTotalFormatted = prettyPrintPrice(userSteamWalletCurrency, (primaryTotal / 100).toFixed(2));
            if (primaryTotalEl !== null) primaryTotalEl.innerText = primaryTotalFormatted;
            else {
                offerPrimarySide.insertAdjacentHTML(
                    'beforeend',
                    DOMPurify.sanitize(`<div class="offerRealTimeTotal">${primaryTotalFormatted}</div>`),
                );
            }
            ...
            if (secondaryTotalEl !== null) secondaryTotalEl.innerText = secondaryTotalFormatted;
                ...
                offerSecondarySide.insertAdjacentHTML(
                    'beforeend',
                    DOMPurify.sanitize(`<div class="offerRealTimeTotal">${secondaryTotalFormatted}</div>`),
                );
            }
        });
    }, 3000);
        ...
        if (itemInOtherOffers) {
            ...
            buttons.forEach((button) => {
                button.addEventListener('click', (event) => {
                    removeOfferFromActiveOffers(
                        getOfferIDFromElement(event.target.parentElement.parentElement.parentElement),
                    );
                });
            });
        }
        ...
        getOffersFromAPI('active').then(
            ({ offers, itemsWithAllInfo }) => {
                ...
                document.getElementById('tradeoffers_summary').innerHTML =
                    DOMPurify.sanitize('<b>Trade offer summary:</b>');

                chrome.storage.local.get('tradeOffersSortingMode', ({ tradeOffersSortingMode }) => {
                    document
                        .querySelector<HTMLInputElement>(`#offerSortingMethod [value="${tradeOffersSortingMode}"]`)
                        .select();
