...
const doTheSorting = (items, itemElements, method, pages, type) => {
    ...
    if (method === 'price_asc') {
        sortedElements = itemElements.sort((a, b) => {
            const priceOfA =
                getItemByAssetID(items, getAssetIDOfElement(a)).price !== undefined
                    ? parseFloat(getItemByAssetID(items, getAssetIDOfElement(a)).price.price)
                    : 0.0;
            const priceOfB =
                getItemByAssetID(items, getAssetIDOfElement(b)).price !== undefined
        ...
        sortedElements = itemElements.sort((a, b) => {
            const stickerPriceOfA =
                getItemByAssetID(items, getAssetIDOfElement(a)).stickerPrice !== null
                    ? parseFloat(getItemByAssetID(items, getAssetIDOfElement(a)).stickerPrice.price)
                    : 0.0;
            const stickerPriceOfB =
                getItemByAssetID(items, getAssetIDOfElement(b)).stickerPrice !== null
    ...
    if (type === 'offer' || type === 'inventory') {
        ...
        sortedElements.forEach((itemElement) => {
            document
                .getElementById(`${type}_slots`)
                .insertAdjacentElement('afterbegin', itemElement.parentNode.parentNode);
        });
